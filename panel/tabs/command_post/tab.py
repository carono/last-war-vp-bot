...
class SharedMissionsPane(_Pane):
    ...
    def _build_listener_bar(self, parent) -> None:
        ...
        # prohibition on, always. It used to be a box, and one shipped OFF — see
        # `SecretTasksTab.rob_candidates` for why that is the wrong shape for a rule
        # whose breach costs a raid rather than raising anything.
...
class CommandPostTab(PanelTab):
    ...
    def web_view(self) -> "dict | None":
        ...
        The ghost finds and the treasures are already written down by the captures
        (`ghost.json`, `treasures.json` in this profile), and the shared missions are
        in the pane that watches for them. So a phone opening this costs three file
    ...
    def _web_shared(self, coords) -> dict:
        ...
        rows = list(getattr(pane, "_rows", {}).values()) if pane is not None else []
        rows.sort(key=lambda m: (not m.get("matched"), -(m.get("level") or 0)))
        items = []
        for mission in rows:
            items.append({
                "text": coords.fmt(0, 0, mission.get("server")),
                "facts": [{"label": "cmdpost.col.level",
                           "value": str(mission.get("level") or 0)}],
                "pill": ("cmdpost.shared.robbed" if mission.get("robbed")
