...
or having run within :data:`COLLECT_WINDOW_SEC`. A harvest made by a thumb in the game
is therefore in the whole-day tally alone, which is a gap and not a lie.
...
#: HOW LONG A GAIN IS STILL THAT HARVEST'S, in seconds after the scenario was last seen
#: running. One press sends a collect per ready building — 36 of them, live — and the
#: client answers with a burst of balance pushes that goes on arriving after the run has
#: ended (`docs/research/resource-collection.md`). Judged too tightly the tail of the
#: burst is credited to nothing; too loosely a truck arriving a minute later would be
#: credited to the base. Forty-five seconds is a burst and no more.
...
class ResourceBook:
    ...
    def from_base(self) -> bool:
        ...
        if COLLECT_ACTION in names:
            ...
            return True
        return bool(self._collect_at) and now - self._collect_at <= COLLECT_WINDOW_SEC
