 ...
 *
 * A SQUAD WITH NO FACE DRAWS ITS NUMBER, and that is deliberate: the `heroId -> icon`
 * table is encrypted on disk, so a hero the live client would not name has no picture at
 * all here rather than a similar one (`docs/research/hero-icons.md`).
 *
      ...
      <div className="squads">
        {slots.map((slot) => {
          ...
          return (
            ...
            >
              <span className="squad-faces">
                {faces.length ? (
                  faces.map((face, i) => <img key={i} src={face} alt="" loading="lazy" />)
                ) : (
