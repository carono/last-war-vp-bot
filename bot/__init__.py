...
Design rules
------------
* No duplication. The wire protocol lives in ``tools/lastwar_proto.py`` and the
  window capture in ``lastwar_bot.perception.capture``; ``bot`` re-exports and
...
for _extra in (_REPO_ROOT / "tools", _REPO_ROOT / "src"):
    _p = str(_extra)
