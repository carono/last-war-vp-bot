...
| `steal_secret_task.py` | **Windows** (Python, Lua daemon) | rob another player's secret task («кража секретки») headlessly — name the target by `--uuid`, by `--coords` (the uuid is resolved the way a marker tap does) or from a `secret_task_capture.py` checkpoint (`--star-max` = the panel's auto-loot rule: starred tasks only, and only at the level `--level-max` asks for — «от 1 до 7» robs 7s and leaves a 6 alone — falling back to the best level found when no `--level-max` is given; `--level-min` / `--level-max` also gate which levels may be robbed at all); `--status` shows the day's five robberies. See [`../docs/research/secret-task-steal.md`](../docs/research/secret-task-steal.md) |
| `ghost_recon_steal.py` | **Windows** (Python, Lua daemon) | rob a ghost-recon squad («Операция Призрак») headlessly — `--list` shows every squad the client knows with the game's own steal verdict, `--all` queues what is robbable, `--status` shows the event day and the five-a-day budget. A different command and budget from `steal_secret_task.py`; see [`../docs/research/ghost-recon-steal.md`](../docs/research/ghost-recon-steal.md) |
...
| `rdp_instance.py` | **Windows** (Python, pywin32) | the **second client, in its own Windows session**, driven from this one over TCP: `--bring-up` creates the session (RDP to this machine by another address — not `localhost`), starts that user's client and a `lua_daemon` on its own port, and leaves the session disconnected. `--status` / `--ping` / `--lua` / `--stop` / `--logoff` / `--restore-console`. The logon is the only step that needs a password, and the tool never holds one: `--save-credential` seals it where only Windows can read it, `--ask` stores nothing and has Windows ask, `--credentials` says which of the two is in place. Any existing tool then drives it with `LW_DAEMON_PORT=47655`; headless abilities only. See [`../docs/research/multi-instance-rdp.md`](../docs/research/multi-instance-rdp.md) and [`../docs/research/rdp-session-credentials.md`](../docs/research/rdp-session-credentials.md) |
