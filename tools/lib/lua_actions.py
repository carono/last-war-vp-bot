...
#:
#: `GotoWorldPos(pos, zoom, nil, nil, serverId)` handed nothing in that slot does not
#: mean «do not switch»: it loads the HOME world. Measured live against a camera standing
#: on a foreign warzone — four arguments, five with `nil`, and `MoveToWorldPoint` too:
#: every one of them took the wire back to the home warzone inside a second, while the
#: same waypoint carrying that warzone's id walked on without leaving it. #2705 measured
#: the same call on a client that was already at home, where the two are indistinguishable.
#:
#: AND THE CLIENT DOES NOT KNOW WHERE IT IS LOOKING. With the camera on a foreign warzone
#: and that warzone's tiles arriving, `WorldFavoDataManager.curServerId`,
#: `WarFlagDataManager.curServerId` and `LuaEntry.Player.serverId` all still name home;
#: `CrossServerUtil.OnCrossServer(id)` flips `GetIsCrossServer()` and writes none of them;
#: a value-search of the whole Lua side for the foreign id finds nothing usable, and a
#: tile's own `serverId` is its OWNER's warzone rather than the world's. So the in-game
#: fallback below is the best the GAME can answer, and it is only a fallback: the panel
#: hears the truth on the wire (`game.server` → `panel/runtime/header.py`) and hands it
#: in as the warzone to walk.
...
def fast_map_sweep(zoom: "int | None" = None, step: "int | None" = None,
    ...
    **LEFT OUT, THE LAP STAYS WHERE THE PERSON PUT THE CAMERA (#2705)**, and it stays
    ...
    # A LAP ALWAYS NAMES A WARZONE (#2727, and see the note above `JUMP_ZOOM`): an empty
    # slot loads the home world, so a lap that named nothing was a lap that went home.
