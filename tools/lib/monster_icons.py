"""Which of the game's own sprites stands for which kind of banner, and where on disk.

The reading half of `tools/extract_monster_icons.py` (#2055). The map is
``tools/data/monster_icons.json`` — kind of banner -> sprite stem — and the sprites are the
client's own art extracted into ``results/monster_icons``, which is git-ignored: the game's
...
ICON_ROOT = os.path.join(_ROOT, "results", "monster_icons")
MAP_PATH = os.path.join(_ROOT, "tools", "data", "monster_icons.json")
...
def stem_for(kind: str) -> str:
    """The sprite stem one kind of banner draws, or ``""`` when it has none."""
