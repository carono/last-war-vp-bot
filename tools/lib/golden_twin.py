...
def pair_recipe() -> str:
    ...
    add("")
    # THE EVENT'S OWN GATE, BEFORE ANYTHING IS SPENT (#2647). The golden zombies belong
    # to «Вторжение зомби» and exist only while it runs; outside its window this driver
    # walked the world scene, a squad refill and a lap of the map to find an empty list.
    add("# THE EVENT'S OWN GATE, AND IT IS THE FIRST THING THE RUN ASKS (#2647). The")
    add("# golden zombies are «Вторжение зомби»'s own monsters and exist only while it")
    add("# runs. One reading answers it (`read_zombie_invasion.md`), and it is the GAME's")
    add("# answer: the invasion manager's own id and the activity row behind it.")
    add("#")
    add("# What overrules a closed reading is the client still SEEING golden zombies: an")
    add("# invasion that has just ended leaves its monsters for their twelve minutes, and")
    add("# «-1» — the base is on screen, so nobody could ask — is not a sight.")
    add("CALL read_zombie_invasion")
    add("IF inv_open == 0")
    add("    IF inv_seen > 0")
    add('        LOG "the invasion reads as over, but the client still knows {inv_seen} golden zombies — hunting what is left"')
    add("    ELSE")
    add('        STOP "the Zombie Invasion is not running — there is nothing to hunt"')
    add("")
    add("IF scene != world")
