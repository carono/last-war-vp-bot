...
do
    ...
    ext.saveimage = function(path, a, x1, y1, x2, y2)
        ...
        C.WriteFile(handle, ffi.cast("const void*", a), h * l, dwbuf, nil)
...
ext.image_copy = function(a, x1, y1, x2, y2)
    ...
    log("fi_a:", table.concat(a, ':'), "x1:", x1, "y1:", y1, "x2:", x2, "y2:", y2)
    ...
    -- Логирование выделенной памяти и размеров
    -- log("Memory allocated for cut_a:", cut_a, "cut_w:", cut_w, "cut_h:", cut_h, "cut_l:", cut_l)

    -- Копируем пиксели
    local y = 0
    for i = y1 * l, y2 * l - 1, l do
        local cut_x = y
        -- Логирование значений в цикле
        -- log("Processing row:", i, "cut_x:", cut_x)

        for j = x1 * 3, (x2 + 1) * 3 - 1 do
            -- Логирование индексов в цикле
            -- log("Copying pixel at i+j:", i + j, "to cut_x:", cut_x)
            --log(a[i + j])
            cut_a[cut_x] = ac[i + j]  -- Записываем пиксель
            cut_x = cut_x + 1
        end
        y = y + cut_l
    end
