...
#
# AND A GIFT IS THANKED FOR, THEN TIDIED AFTER (#2603). What the game calls the thing
# is «Счастливый подарок» (`item_name_995001`, Lucky Gift), and a person who takes one
# does two more things by hand: presses the like, and shuts the window the client opened
# over the chat. Both are done here, two seconds after the press, so neither delays the
# race the press itself is in.
#
#   * THE LIKE is `InteractiveUtil.DoSendMessage(<sharer uid>, 61, <message seqId>, '')`
#     — `thumbs.up`, with `ThumbsUpType.AllianceLuckSiphonRedPacket = 61`. That the 61 is
#     this gift and not another is the client's own word for it:
#     `GetLangKeyByThumbType(61)` answers `conveyLuck_like_tips` — «Спасибо за Счастливый
#     подарок, которым ты поделился». **The types are not free**: the serialiser wants the
#     uid and the sequence as STRINGS and refuses a number before a byte leaves
#     (`SFSDataSerializer.lua:55: attempt to get length of a number value`), the same way
#     the open above needs its fourth field.
#   * THE PROOF IS A SPENT CHARGE, never a send that did not raise. Likes of this kind
#     are capped at ten a day and the client counts them itself
#     (`GetCanThumbsUpCount(61)`), so the count is read before and after: it drops by one
#     or the row says the like did not go through. Measured live on the ordinary chat like
#     (type 2, the same command): 20 → 19 (#2603).
#   * `InteractiveUtil.TryThumbsUp` is deliberately NOT used. It answers `true` and sends
#     nothing at all — measured three ways, including from the main thread through
#     `DelayInvoke` with a wrapper on `SFSNetwork.SendMessage` watching: the wire stayed
#     empty and the day's count did not move.
#   * THE WINDOW is closed with the client's own `Ctrl:CloseSelf()` on
#     `LWUIRedPacketDetails` and `LWUIRedPacketOperation` — never `DestroyAllWindow`,
...
# WHERE THE CALLBACK IS PARKED, AND WHY NOT A GLOBAL (#2656). It used to be
# `_G.__lw_rp_take`, and on a live client that line does not take: the build guards its
# own globals (`Global/GlobalProtect.lua`, an `__newindex` that REFUSES a new name and
# only logs it — `Lua 全局变量 '…' 不可<新增/修改>`, «cannot be added/modified»). The
# refusal is silent on our side: the chunk carries on, the wrapper installs, `B.on` goes
# true, and then every message calls `pcall(nil, …)` and takes nothing. That is exactly
# what the live log showed — «слышал 0, забрал 0» for weeks with the ear reported armed.
# So everything this ability owns hangs off ONE table on `DataCenter`, which is an
# ordinary field and is not guarded. Never park anything of ours on `_G`.
LUA (function() local B = DataCenter.__lw_rpw if B == nil then B = {on = false, heard = 0, taken = 0, failed = 0, skipped = 0, liked = 0, likeFailed = 0, closed = 0, lastMs = -1, bestMs = -1, err = '', rows = {}} DataCenter.__lw_rpw = B end if B.liked == nil then B.liked = 0 B.likeFailed = 0 B.closed = 0 end local CM = package.loaded['Chat.Model.ChatMessage'] if type(CM) ~= 'table' or type(CM.onParseServerData) ~= 'function' then B.on = false B.err = 'no chat class' return end B.take = function(msg) local B2 = DataCenter.__lw_rpw if B2 == nil then return end local post = nil pcall(function() post = tonumber(msg.post) end) if post ~= 611 then return end B2.heard = B2.heard + 1 local t0 = os.clock() local function note(word) B2.rows[#B2.rows + 1] = word while #B2.rows > 20 do table.remove(B2.rows, 1) end end local rj = package.loaded['rapidjson'] if type(rj) ~= 'table' then B2.failed = B2.failed + 1 note('no-json') return end local s = '' pcall(function() s = tostring((msg.extra or {}).customJsonParam or '') end) local ok, d = pcall(function() return rj.decode(s) end) if not ok or type(d) ~= 'table' or d.uuid == nil then B2.failed = B2.failed + 1 note('unreadable') return end local key = tostring(d.uuid) if B2.seen == nil then B2.seen = {} end if B2.seen[key] then B2.skipped = B2.skipped + 1 note('already') return end local M = DataCenter.RedPacketManager local inst = M and (M.Instance or M) local got, max = 0, 0 pcall(function() got = math.floor((inst:GetRedPacketGetNum() or 0) + 0) end) pcall(function() max = math.floor((inst:GetRedPacketGetMaxNum() or 0) + 0) end) if max > 0 and got >= max then B2.skipped = B2.skipped + 1 note('quota ' .. got .. '/' .. max) return end local now = 0 pcall(function() now = math.floor((UITimeManager:GetInstance():GetServerTime() or 0) + 0) end) if now <= 0 then now = os.time() * 1000 end local exp = math.floor((tonumber(d.expiredTime) or 0) + 0) if exp > 0 and exp <= now then B2.skipped = B2.skipped + 1 note('expired') return end local home = 0 pcall(function() home = math.floor((tonumber(tostring(LuaEntry.Player.serverId)) or 0) + 0) end) local theirs = math.floor((tonumber(d.redPacketServerId) or tonumber(d.serverId) or 0) + 0) if home > 0 and theirs > 0 and home ~= theirs then B2.skipped = B2.skipped + 1 note('another server') return end local room = '' pcall(function() room = tostring(msg.roomId or '') end) local chat = 0 if room:find('^alliance') then chat = 2 elseif room:find('^world') then chat = 1 elseif room:find('^season') then chat = 3 end if chat == 0 then B2.skipped = B2.skipped + 1 note('room ' .. (room:match('^(%a+)') or '?')) return end local cfg = math.floor((tonumber(d.packetId) or 0) + 0) B2.seen[key] = true local srv = math.floor((tonumber(d.redPacketServerId) or tonumber(d.serverId) or 0) + 0) local ok2, why = pcall(function() SFSNetwork.SendMessage(MsgDefines.OpenRedPacket, tostring(d.uuid), cfg, chat, srv) end) local ms = math.floor((os.clock() - t0) * 1000) if ok2 then B2.taken = B2.taken + 1 B2.lastMs = ms if B2.bestMs < 0 or ms < B2.bestMs then B2.bestMs = ms end note('took cfg=' .. cfg .. ' chat=' .. chat .. ' srv=' .. srv .. ' ' .. ms .. 'ms') local uid = '' pcall(function() uid = tostring(msg:getSenderUid() or '') end) local seq = '' pcall(function() seq = tostring(msg:getSeqId() or '') end) local function shut() local shutn = {} for _, n in ipairs({'LWUIRedPacketDetails', 'LWUIRedPacketOperation'}) do pcall(function() local w = UIManager.Instance:GetWindow(n) if w and w.Ctrl and w.Ctrl.CloseSelf then w.Ctrl:CloseSelf() shutn[#shutn + 1] = n end end) end if #shutn > 0 then B2.closed = B2.closed + 1 note('окно закрыто: ' .. table.concat(shutn, ',')) end end local function left61() local n = -1 pcall(function() n = math.floor((InteractiveUtil.GetCanThumbsUpCount(61) or 0) + 0) end) return n end pcall(function() TimerManager:GetInstance():DelayInvoke(function() local mine = (uid ~= '' and uid ~= 'nil' and uid ~= '0') local left0 = left61() if not mine then note('лайк некому — подарок без отправителя') shut() return end if left0 == 0 then note('лайк не поставлен — дневной лимит') shut() return end local okl, whyl = pcall(function() InteractiveUtil.DoSendMessage(uid, 61, seq, '') end) if not okl then B2.likeFailed = B2.likeFailed + 1 note('лайк отказан: ' .. tostring(whyl)) shut() return end TimerManager:GetInstance():DelayInvoke(function() local left1 = left61() if left1 >= 0 and left0 > left1 then B2.liked = B2.liked + 1 note('спасибо сказано, лайков осталось ' .. left1) else B2.likeFailed = B2.likeFailed + 1 note('лайк не прошёл — счётчик не сдвинулся (' .. left0 .. '/' .. left1 .. ')') end shut() end, 1.5) end, 2) end) else B2.failed = B2.failed + 1 B2.err = tostring(why) B2.seen[key] = nil note('refused') end end if B.wrapper ~= nil and CM.onParseServerData == B.wrapper then B.on = true return end B.orig = CM.onParseServerData B.wrapper = function(self, ...) local r = {B.orig(self, ...)} pcall(B.take, self) return table.unpack(r) end CM.onParseServerData = B.wrapper B.on = true end)()
