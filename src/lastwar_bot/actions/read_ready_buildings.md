...
#
# **AND A SLOT WHOSE TIMER HAS RUN OUT COUNTS TOO, WHATEVER ITS `state` SAYS (#2641).**
# The flip to `Finish` is the client's own, and the client can be minutes behind — it
# drops the `uT` the server sends it after a speed-up, so a construction that is closed
# on the server sits in `Work` with an `endTime` in the past. A timer that has run out is
# the honest definition either way, and the arbiter of an actual claim stays the server:
# `open_ready_buildings.md` sends and reports what came back.
#
...
# `+`. **One entry per item id**: the piece that overshoots is folded into the entry of
# its own denomination rather than standing beside it, so a row says «5018×1 мин» and
# never «5017×1 мин + 1×1 мин». Nothing is spent by reading it; spending it is
# `finish_building.md`, which works the same parcel out again for itself at the moment
# of the press.
#
