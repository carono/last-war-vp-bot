...
READ_LUA (function() local info = nil local M = DataCenter and DataCenter.TacticalWeaponManager if M ~= nil then pcall(function() info = M:GetTacticalWeaponInfo(1000) end) if type(info) ~= 'table' then pcall(function() for _, v in pairs(M.tacticalWeaponInfos or {}) do info = v end end) end end if type(info) ~= 'table' then return 'lv=0 max=0 why=no-drone' end local row = info.levelTemplate if type(row) ~= 'table' then pcall(function() row = info:GetLevelTemplate() end) end local D = DataCenter.ItemData local have = {} if D ~= nil then pcall(function() for _, v in pairs(D.ItemInfos or {}) do local id = math.floor(tonumber(v.itemId) or 0) have[id] = (have[id] or 0) + math.floor(tonumber(v.count) or 0) end end) end local bits = {} if type(row) == 'table' and type(row.cost_resItem) == 'table' then for _, c in pairs(row.cost_resItem) do if type(c) == 'table' then local id, n = math.floor(tonumber(c.id) or 0), math.floor(tonumber(c.value) or 0) bits[#bits + 1] = id .. ':' .. (have[id] or 0) .. '/' .. n end end end local function ask(name) local v = nil local ok = pcall(function() v = info[name](info) end) if not ok then return -1 end if v == true then return 1 end if v == false then return 0 end return math.floor(tonumber(v) or -1) end return 'lv=' .. tostring(math.floor(tonumber(info.level) or 0)) .. ' max=' .. tostring(math.floor(tonumber(info.maxLevel) or 0)) .. ' capped=' .. tostring(ask('IsReachLevelLimit')) .. ' topped=' .. tostring(ask('IsReachMaxLevel')) .. ' pays=' .. tostring(ask('HasResItemToUpgrade')) .. ' cost=' .. table.concat(bits, ',') end)() INTO drone_state
