...
READ_LUA (function() local B = DataCenter.__lw_fww local G = DataCenter.LWFireworkGiftManager local got = 0 for _ in pairs((G and G.giftUuid2TimeTable) or {}) do got = got + 1 end if not B then return 'on=0 pushes=0 presses=0 taken=0 failed=0 foreign=0 lastMs=-1 bestMs=-1 onRecord=' .. got .. ' rows=[]' end local rows = table.concat(B.rows, '; ') B.rows = {} return 'on=' .. (B.on and 1 or 0) .. ' pushes=' .. B.pushes .. ' presses=' .. B.presses .. ' taken=' .. B.taken .. ' failed=' .. B.failed .. ' foreign=' .. (B.foreign or 0) .. ' lastMs=' .. B.lastMs .. ' bestMs=' .. B.bestMs .. ' onRecord=' .. got .. (B.err ~= '' and (' err=' .. B.err) or '') .. ' rows=[' .. rows .. ']' end)() INTO watch
