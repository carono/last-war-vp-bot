...
# game's own VM (`DataCenter.__lw_fb_lane`) and every chunk here reads it from there.
#
...
READ_LUA (function() local M=DataCenter.ActFrontBreakSundayDataManager local d=M.dataDict[M:GetFirstActId()] M.requestingEnterStage=false local nxt=tonumber(d.nextStageId) or 0 if nxt<=0 then return 0 end DataCenter.__lw_fb_stage=nxt local ok=pcall(function() M:RequestToEnterStage(nxt) end) return ok and nxt or 0 end)() INTO fb_stage

IF fb_stage == 0
    FAIL "frontline breakthrough: the client would not enter a stage"

READ_LUA (function() local B=DataCenter.LWBattleManager local L=B:GetCurBattleLogic() if not L then return 0 end if tonumber(L.levelId)~=tonumber(DataCenter.__lw_fb_stage) then return 0 end return B:IsBattleFinish() and 0 or 1 end)() INTO fb_running
...
READ_LUA (function() local L=DataCenter.LWBattleManager:GetCurBattleLogic() local T=L and L.team if not T then return (DataCenter.__lw_fb or {}).peak or 0 end return (T.teamUnitCount or 0)+(T.overflowUnitCount or 0) end)() INTO fb_left
READ_LUA ((DataCenter.__lw_fb or {}).peak or 0) INTO fb_peak
READ_LUA (function() local st=DataCenter.__lw_fb or {} local RT=DataCenter.__lw_fb_routes or {} local sid=st.stage or 0 local left=0 local L=DataCenter.LWBattleManager:GetCurBattleLogic() local T=L and L.team if T then left=(T.teamUnitCount or 0)+(T.overflowUnitCount or 0) end local b=RT[sid] local was=(b and b.best) or 0 if left>was and left>0 then local cp={} for k,v in pairs(st.rec or {}) do cp[k]=v end RT[sid]={best=left,r=cp,runs=((b and b.runs) or 0)+1} DataCenter.__lw_fb_routes=RT return left end if b then b.runs=(b.runs or 0)+1 end return -was end)() INTO fb_route
READ_LUA ((DataCenter.__lw_fb or {}).moves or 0) INTO fb_moves
READ_LUA ((DataCenter.__lw_fb or {}).frames or 0) INTO fb_frames
READ_LUA (function() local st=DataCenter.__lw_fb or {} local t=st.tail or {} local o={} local n=#t for i=1,n do local j=((st.ti or 0)+i-1)%n+1 o[#o+1]=t[j] end return table.concat(o,' | ') end)() INTO fb_tail

READ_LUA (function() local U=UIManager.Instance local M=DataCenter.ActFrontBreakSundayDataManager if U:IsWindowOpen(UIWindowNames.UIBattleResultFrontBreakSundayVictory) then DataCenter.__lw_fb_won=1 return 1 end if U:IsWindowOpen(UIWindowNames.UIBattleResultFrontBreakSundayDefeat) then DataCenter.__lw_fb_won=0 return 0 end local n=tonumber(M.dataDict[M:GetFirstActId()].nextStageId) or 0 if n~=tonumber(DataCenter.__lw_fb_stage) then DataCenter.__lw_fb_won=1 return 1 end DataCenter.__lw_fb_won=0 return -1 end)() INTO fb_win
