...
#
# **A list that has gone stale is NOT a refusal.** The five opponents ride on the battle
# reply, and one of the rows can be gone from the server's list by the time the next
# round tries it — the server then answers `new_arena_tips_38 not in battleList` and the
# battle costs nothing. Two of those in a row used to end the run in an error and throw
# away the rest of the day's attempts. The recipe now asks for the list again (the free
# `isRefresh = 0`, which costs nothing) and fights on: up to three re-asks a run, each
# costing one empty round, and only a refusal that is NOT about the list counts towards
# the two strikes that stop a run.
#
