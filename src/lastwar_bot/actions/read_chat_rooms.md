...
#   rooms   one room per line: `<id>\t<name in hex>\t<messages held>`. The name is hex
...
READ_LUA (function() local I = package.loaded["Chat.ChatInterface"] if type(I) ~= "table" then return "" end local ok, mgr = pcall(function() return I.getRoomMgr() end) if not ok or type(mgr) ~= "table" then return "" end local function hex(s) if type(s) ~= "string" or s == "" then return "" end return (s:gsub(".", function(c) return string.format("%02x", string.byte(c)) end)) end local out = {} for key, rd in pairs(mgr.roomDatas or {}) do local msgs, name = 0, "" if type(rd) == "table" then if type(rd.msgs) == "table" then msgs = #rd.msgs end for _, f in ipairs({"roomName", "name", "title", "groupName"}) do local v = rd[f] if type(v) == "string" and v ~= "" then name = v break end end end out[#out+1] = tostring(key) .. "\t" .. hex(name) .. "\t" .. msgs end return table.concat(out, "\n") end)() INTO rooms
