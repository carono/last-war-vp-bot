...
# STOPPING IT IS THE LAP'S OWN STOP (#2739). The two jumps and the readings behind them
# are scheduled inside the game exactly as a lap's waypoints are, so they carry the same
# run token: «Остановить обход» — `actions/stop_map_sweep.md` — bumps it and the second
# jump and every pending reading disown themselves. Without that a measurement started by
# mistake walked the camera to the far corner and back whatever anybody pressed.
#
ARGS x = 0
ARGS y = 0
ARGS zoom = 600
# WHICH WARZONE THE TWO JUMPS ARE ON (#2727). Left at 0 the chunk asks the client,
# and an EMPTY warzone slot is not «stay» — it loads the home world, so the
# measurement used to drag a person standing on another warzone home, twice a run.
