...
LUA DataCenter.__lw_gold2_cluster = {cluster}
...
# THE FIRST TARGET IS CHOSEN BEFORE THE LOOP, AND EVERY LATER ONE WHILE THE SQUAD IS
# STILL IN THE AIR (#2390). The bricks and the facts they stand on have not changed —
# nothing is judged before the march that would have killed it has landed, nothing is
# sent at a target that has not been checked — but the CHOOSING never needed the squad to
# be standing still. It needs to know where the squad will be, and that is the tile the
# march in flight is aimed at, which is known the moment the order leaves.
#
# Measured on the lap this task was opened for: judging cost 2.0 s, choosing 1.6 s and
# sending 4.4 s, all of it between «the march is over» and «the next order is away», with
# the squad standing idle for the lot. Choosing — and with it the look at a far target's
# district, which is a two-second camera flight — moves into the shadow of the flight,
# where it is free.
...
WHILE go == 1 LIMIT 200
    ...
    IF go == 1
        # THE ORDER GOES OUT FIRST, AND THE LOOKING IS DONE WHILE IT FLIES (#2390). The
        # window in which the squad's own march can be re-aimed where it stands is
        # seconds wide — measured off the operator's own hand-driven chain on the wire,
        # 0.1 s to 3.4 s after each arrival — and judging the kill first spends exactly
        # that window on two scans and a camera move. So the next zombie, chosen while
        # the march was still in the air, is ordered the moment the squad lands, and the
...
# THE LAST ORDER IS JUDGED TOO, WHEN IT IS CHEAP TO (#2390). The chain judges one lap
# behind — the send parks the zombie it has just fought in `p.judge` — so the target of
# the FINAL order has nobody behind it to look at it. It is looked at here instead, and
# only when the squad's march is already over: a run whose last march is still in the air
# would otherwise sit in front of it for a minute and a half with nothing left to do.
...
IF reaimable == 0
    READ_LUA (function() local p = DataCenter.__lw_gold2 or {} p.judge = p.judge or p.hit DataCenter.__lw_gold2 = p return 1 end)() INTO last_judged
    CALL golden2_judge_the_kill
