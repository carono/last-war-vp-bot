...
ARGS y = 0
ARGS server = 0
# …and the tile's OWN packed pointId, when the caller has it. The wire carries one on
# every ghost tile (`point_id`), and it is the id the game itself keys a point by —
# `SceneUtils.TilePosToIndex(x, y)` is only our reconstruction of it, and a
# reconstruction that packs differently asks about the wrong square (#2010).
ARGS pid = 0
...
LUA local pid = ({pid} > 0) and {pid} or (({x} > 0 and {y} > 0) and SceneUtils.TilePosToIndex(CS.UnityEngine.Vector2Int({x},{y})) or 0) if pid > 0 then DataCenter.__lw_gate_pid = pid pcall(function() SFSNetwork.SendMessage("world.get.detail.new", pid, {server}, 0, 29, "") end) end

WAIT 3

READ_LUA (function() local M=DataCenter.ActGhostreconManager local id=tostring({uuid}) local out={} local function put(k,v) out[#out+1]=k..'='..tostring(v) end local t=nil for _,list in ipairs({M.taskList or {}, M.allianceTaskList or {}}) do for _,x in ipairs(list) do if tostring(x.uuid)==id then t=x end end end put('found', t~=nil and 1 or 0) if t then local me=tostring(LuaEntry.Player.uid) put('mine', tostring(t.ownerId)==me and 1 or 0) local n=0 for _,s in ipairs(t.stealList or {}) do n=n+1 end put('looted', n) local tpl=M:GetTaskTemplate(t.cfgId) put('max', tpl and (tonumber(tpl.stealMaxtimes) or 3) or '-') local srv=t.ownerServer or t.targetServer put('srv', srv) put('in_range', (srv and (M.dispatchStealRange or {})[srv]==true) and 1 or 0) local ok,st=pcall(function() return M:GetPointStealType(t.cfgId, t.completionTime, {}) end) put('point_type', ok and tostring(st) or 'err') end local cfg=M:GetNowSettingCfg() local cap=tonumber(cfg and cfg.stealCount) or 0 local used=tonumber(M.stealTimes) or 0 put('left', cap-used) local pid=DataCenter.__lw_gate_pid or 0 if pid > 0 then put('pid', pid) local ok2,d=pcall(function() return DataCenter.WorldPointDetailManager:GetDetailByPointId(pid) end) put('detail', (ok2 and d) and 1 or 0) if ok2 and d then put('d_uuid', tostring(d.uuid or 0)) put('d_type', tostring(d.pointType or '-')) put('t', (function() local ti=d.taskInfo if not ti then return 'none' end local ks={} for k,v in pairs(ti) do ks[#ks+1]=tostring(k)..'='..(type(v)=='table' and 'T' or tostring(v)) end table.sort(ks) return table.concat(ks,',') end)()) put('verdict', (function() local ti=d.taskInfo if not ti then return '-' end local okv,st=pcall(function() return M:GetPointStealType(ti.cfgId, ti.completionTime, ti.stealList or {}) end) return okv and tostring(st) or 'err' end)()) put('cansteal', tostring(GhostreconPointStealType and GhostreconPointStealType.CanSteal or '-')) put('d_keys', (function() local ks={} local ok3=pcall(function() for k,v in pairs(d) do ks[#ks+1]=tostring(k)..':'..type(v) end end) if not ok3 then local mt=getmetatable(d) if mt then for k,_ in pairs(mt) do ks[#ks+1]='mt.'..tostring(k) end end end table.sort(ks) return table.concat(ks,',') end)()) end end put('cached', (function() local n=0 for _ in pairs((DataCenter.WorldPointDetailManager or {}).worldPointDetailList or {}) do n=n+1 end return n end)()) put('type29', (function() if not WorldPointType then return 'no_enum' end for k,v in pairs(WorldPointType) do if tonumber(v)==29 then return tostring(k) end end return 'none' end)()) put('range_size', (function() local n=0 for _ in pairs(M.dispatchStealRange or {}) do n=n+1 end return n end)()) return table.concat(out,' ') end)() INTO gate
