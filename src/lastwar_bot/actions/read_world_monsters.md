...
#               Zombie Boss) and **8** is the Doom line («Роковая Элита»). 0 means the
#               row carried no config id to ask about — the same split
#               `join_rally.md` sorts its banners by.
#   * `level` — the monster's level, 0 when neither the config nor a label gave one.
#   * `kind`  — the drawn object's own name with `(Clone)` taken off, which is the only
#               thing a roaming monster says about itself before it is selected.
#
