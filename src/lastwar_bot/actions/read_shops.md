...
#
#             THE HONOUR (type 40) IS NAMED BY NOBODY THE MANAGER ASKS (#2832):
#             `GetResourceNameByType(40)` and `GetResourceIconByType(40)` are both
#             empty. The picture is in the client's own config all the same — the
#             honour is row 1005 of `aps_resources` (`icon = zyf_xunzhang_icon`,
#             `name = 457008`, which the game's locale tables say is «Очки чести» /
#             «Honor Points»), so `APS` below maps the type onto that row and the
#             sprite comes out of the game rather than out of this file. The WORD is
#             the panel's own key `shop.money.honor`, copied out of those same tables
