...
#
# The bag's own `count` is the CLIENT's copy and it does not fall the moment a parcel
# leaves — the server's answer arrives later. A run that plans its next parcel off that
# copy picks the same id again, spends pieces it no longer owns, gets nothing back and
# reads the still score as «this phase does not pay», stopping with one denomination
# spent and the rest of the bag untouched. So the run keeps its OWN tally of what it has
# handed over (`used`) and subtracts it from every count it reads: the pool empties as it
# is spent and the sort walks on to the next size, then to the universal ones.
#
...
LUA DataCenter.__lw_arms_sp = {cap = (tonumber("{minutes}") or 0) * 60, spent = 0, sends = 0, own_num = 0, own_sec = 0, uni_num = 0, uni_sec = 0, kinds = {}, used = {}, sc0 = -1, sc_start = -1, rate_live = 0, need = 0, top = 0, paid = 1, why = 'nothing tried'}
