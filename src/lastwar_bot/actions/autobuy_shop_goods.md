...
#
# AND THE PURSE IS ASKED BEFORE ANYTHING IS SENT — a ceiling is not a balance. Diamonds
# are `LuaEntry.Player.gold`; every other currency is `LuaEntry.Resource:GetCntByResType`.
# A balance that reads as ZERO is treated as UNKNOWN and never trims: not every currency
# of every shelf is in that table (measured: the expedition shelf's is not), and a gate
# that cannot tell «empty» from «cannot see» would silently stop buying things the
# account can well afford. `CheckCostEnough` stays as the game's own second opinion.
#
