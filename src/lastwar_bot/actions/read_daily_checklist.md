...
#                    one for taking part.
#   alstar_scratch   1 when the ceremony's scratch card has been taken, 0 while it
#                    is still there, and a DASH when there is no ceremony this
#                    client knows of (#2847). The card is the ceremony's third
#                    reward and the server's own `scratchClaimed` is the answer —
#                    a dash rather than a «0» is what keeps the row from reading
#                    «не забрана» on a week with no ceremony in it.
