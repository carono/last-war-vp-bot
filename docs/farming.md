...
🟩 27 done · 🟨 32 partly · 🟥 44 not automated
<!-- progress:end -->
...
- 🟡 Switching between the characters on one login — a panel tab lists every character this login can play, each with its server, zone, base level and name, highlights the one in play, and puts a one-press «Switch» beside every other one. It is the characters that are listed and nothing else: a character that has changed server appears once, on the server it is on now, and one that was made and never played is left out, so the tab no longer shows more characters than the account has. A switch reconnects the client to that character, exactly what tapping the row in the game's own account screen does; it asks first, since it drops the current session. Reading the list is proven against the live game; the switch builds and sends the game's own reconnect, but a whole changeover has not been played through in a real session yet
