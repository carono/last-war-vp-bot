...
- 🟡 Being told a rally has gone out, and joining it without hunting for it — the moment one appears the panel says so and rings, once per rally rather than once per event; one button joins with the squads ticked right there on the «Ралли» tab, and a switch makes it join by itself as the alert lands — with the rally monitor off, if you like: the monitor only writes the armies down for later, and joining does not depend on it. Which squads may go is the setting it always looked like, and it now sits beside the switch that spends it. THE JOIN ITSELF IS NOT RELIABLE and the mark says so: it was seen working once in a live game — the alliance's own participant list came back one name longer with the player in it — and on the next day's rallies the same press went out and nothing joined, several times over, with the game accepting the send and quietly doing nothing. What is fixed is that the bot no longer pretends: a run that joined nothing now says so and counts as a failure, and a quiet minute with no rally out says that instead. Two things stay the person's: which rallies are worth joining, and the daily cap
