...
🟩 30 done · 🟨 32 partly · 🟥 44 not automated
<!-- progress:end -->
...
- 🟡 Turning the picture down so the client stops loading the video card — an errand that tells the game to draw ten frames a second at its lowest quality in a small window, which costs the card about a fifth of what a client at full quality costs it, and slows nothing the bot does. Nobody is looking at that picture anyway: the bot reads the game rather than watching it. The same errand hands the client back to a person — 60 frames a second, full quality, full-size window — so it can be turned on for the night and off in the morning. Hiding the window instead does not work and never did: a game that is minimised, covered by another window, or running where there is no screen at all draws exactly as many frames as one you are looking at, and costs the card MORE, not less. The frame rate and the quality have to be set again after every restart, so the errand wants a period rather than a single press; the window size is the one part that stays put by itself. Nothing schedules it yet, and it has not been run through a whole session
