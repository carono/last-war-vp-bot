...
🟩 31 done · 🟨 32 partly · 🟥 44 not automated
<!-- progress:end -->
...
- ✅ Turning the picture down so the client stops loading the video card — a switch on the settings page, «Стандартное» or «Низкое»: on low the game draws ten frames a second at its lowest quality in a small window, which costs the card about a fifth of what a client at full quality costs it, and slows nothing the bot does. Nobody is looking at that picture anyway — the bot reads the game rather than watching it. The switch belongs to the account, so the one playing in the background can be economising while the one you are watching is not. Switching back gives you the picture YOU had, not a guess: it reads and remembers your settings before it changes them. Beside the switch is what the client is really drawing right now, asked of the game itself rather than taken on trust, because a game that restarted comes back at full quality without telling anyone. Hiding the window instead does not work and never did: a game that is minimised, covered by another window, or running where there is no screen at all draws exactly as many frames as one you are looking at, and costs the card MORE, not less. Proven live on both clients, there and back
- 🟡 Keeping the picture down without being asked — the switch above is a press, and the frame rate and quality do not survive the game restarting, so a client that crashed comes back at full quality with the switch still saying «Низкое». Giving the errand «set_graphics_load» a period in the schedule covers it, and nothing sets that up for you yet. The window size is the one part that stays put by itself
