...
- 🟡 Turning the picture down so the client stops loading the video card — an errand that tells the game to draw ten frames a second at its lowest quality in a small window, which costs the card about a fifth of what a client at full quality costs it, and slows nothing the bot does. Nobody is looking at that picture anyway: the bot reads the game rather than watching it. The same errand hands the client back to a person — 60 frames a second, full quality, full-size window — so it can be turned on for the night and off in the morning. The settings do not survive a restart, so it belongs right after the game is started; nothing schedules it by itself yet, and it has not been run through a whole session
