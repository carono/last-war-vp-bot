...
🟩 33 done · 🟨 38 partly · 🟥 44 not automated
<!-- progress:end -->
...
- 🟡 The three lists as pages, not one squeezed window — the starred raids, the alliance's own tasks and the Ghost Operation squads each get a page of the «Секретки» tab, and one strip of buttons follows whichever page is in front, so «Собрать» is never aimed at a row nobody can see. The raid page keeps the tiles on your own server out of the way unless you ask for them — the raids worth a march are the ones abroad — and says how many it is holding back, so a short list is never mistaken for a tab that read nothing. The ghost page is a reading: every squad the game knows, with its rank, its countdown and the game's own word on whether it can be robbed, plus whether the event is on today and how many robberies are left; your own empty dispatch slots are not drawn as targets, and your own squads say what they are doing rather than pretending to be somebody to rob. The robbery itself stays where it was, on the «Командный пункт» tab. Seen live with the event open — one squad out, its countdown running — but not yet lived with through a whole event day
