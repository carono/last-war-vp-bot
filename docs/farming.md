...
- 🟡 Treasure notifications, and answering one on its own — when somebody in the alliance shares a chest into chat, the panel hears it in the same second the game does and, with the switch on, works it end to end without anybody looking: the nearest free squad marches onto the chest, and the gift is taken as soon as the chest is dug. It never sweeps the map for chests — the announcement carries the place, the server and which chest it is, so there is nothing to hunt for; what it checks every few seconds is its own note of what it has heard, which costs the game nothing. It works several chests at once, nearest first, one squad each, and it says per chest what it did — sent, still being dug, taken, or gone from the map. A squad that reads as empty is asked about rather than written off, the same way the rally join does it, and a chest the alliance digs on another server is marched at the same way. Which squads it may spend is yours to set, and the whole thing is off until switched on. Everything around it is proven on a live client — the ear, the check, the run, the squad question, the reading of the announcement inside the game itself — but no radar event has put a chest out since it was built, so the march and the take have never gone at a real chest
