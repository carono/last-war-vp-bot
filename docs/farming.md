...
- 🟡 Treasure notifications, and answering one on its own — the panel hears a chest the moment the game does and, with the switch on, works it without anybody looking: the nearest free squad marches onto it, and the gift is taken as soon as it is dug. It hears a chest TWO ways, which turned out to matter: somebody sharing it into alliance chat, and — because sharing is something a person does and often nobody does it — the alliance's own digging, which names the chest even when nothing was shared. A chest heard the second way can only be collected, not marched at, and it is. The map is never swept for chests: what is checked every few seconds is the panel's own note of what it has heard, which costs the game nothing. Several chests at once are worked nearest first, one squad each, and each one says what happened to it — sent, still being dug, taken, or gone from the map. A squad that reads as empty is asked about rather than written off, and a chest on another server is marched at the same way. **Taking the gift is proven in a live game** — the reward window came up, the panel closed it, and the haul was there. **Sending the squad is not**: the one live chest was already dug by the time it was tried, so the dig march has still never gone out from the bot. Which squads it may spend is yours to set, and the whole thing is off until switched on
...
| Dig and collect treasures | 🟡 a chest shared into alliance chat is answered on its own — nearest free squad out, gift taken once it is dug — plus the older ask-the-server page; no chest has been out to try either on |
| Fireworks | ❌ |
