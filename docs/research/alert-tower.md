...
| `SendRewardReceiveMessage()` | **nothing — the call returns without an error and puts NO message on the socket** (re-measured 2026-09-06 with an ear on `SFSNetwork.SendMessage`, which caught `idle.game.main` from the same probe seconds earlier). The line in §5 that used to claim it fetched the loot was written from the SEND returning `true`, which is exactly the trap `CLAUDE.md` names |
| `SendIdleGameEndMessage()` | **this is what pays a finished run**: `idle.game.end` leaves the client, the server takes it with no `errorCode`, and `passNode` 480 → `0` with `startTime`/`endTime` → `0`. Until it is sent the march stands on the board unpaid and its tasks stop moving (#2585) |
