...
| **Out** — the two settings stores that have NOT moved | `config.json` — a settings store AND the thing that says a directory IS a profile (#1306): the panel lists, repairs and refuses profiles by looking for it, so moving it means redefining what a profile is. `profiles/settings.json` — panel-wide, and there is no machine-wide database. Both are questions for the person rather than a decision to take in passing |
