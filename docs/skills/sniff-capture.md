...
| **Game endpoint** | One plain-TCP connection to the gateway — **`:10012` on the current client**, `:17935` on older ones, and it will move again. The IP is dialled without DNS and changes per session, so never write either half down: the tools read the port off the running client (`map_capture.detect_game_ports`), and `--port` / `--all-tcp` are the manual overrides. The client ALSO keeps a chat/control channel on a port of its own — see [`../research/protocol.md`](../research/protocol.md) §1 before concluding «the port moved». |
| **Game running?** | Not «a socket exists» — **`game_link.probe()`**, which is the one rule for it: `/mnt/c/Python312/python.exe -c "import sys; sys.path[:0]=['tools','tools/lib']; import game_link; print(game_link.probe())"`. `link='online'` means the game's own conversation has a live socket; `link='lost'` means it is half-closed and the client is stranded — it will look perfectly healthy and capture nothing, which is exactly what an idle account looks like. A stranded client needs a restart, not a longer capture. |
