"""The panel-wide numeric-field rule (panel/widgets.py):
...
from panel.widgets import is_number, filter_number  # noqa: E402
...
def test_the_widget_wires_validation_and_a_filtering_paste():
    try:
        ...
        return
    try:
        from panel.widgets import NumericEntry, numeric_spinbox
        root = tk.Tk()
        ...
        return
    try:
        entry = NumericEntry(root)
        entry.pack()
        assert entry.cget("validate") == "key", "typing validation not installed"
        ...
        entry.delete(0, "end")
        entry._numeric_paste()
        assert entry.get() == "1234", entry.get()
        ...
        entry._numeric_paste()
        assert entry.get() == "9009", entry.get()

        # Signed field keeps a leading minus through a paste.
        signed = NumericEntry(root, signed=True)
        signed.pack()
        root.clipboard_clear()
        root.clipboard_append("-1x2y")
        signed.delete(0, "end")
        signed._numeric_paste()
        assert signed.get() == "-12", signed.get()
    finally:
        root.destroy()
