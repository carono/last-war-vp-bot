r"""The scenario runner in the «Develop» tab — editing a script, and driving a run.

The list/editor/runner used to be its own «Сценарии» tab; it joined «Develop»
(task #1240) and kept every one of its behaviours, which is what this file still
pins. Two things were added on top of "pick a script and run it" (task #1118), and
both can go wrong quietly:
...
are never written to.
    ...
    C:\Python312\python.exe tests\test_panel_develop_scenarios.py
    python3 tests/test_panel_develop_scenarios.py        # SKIP without tkinter
"""
...
def _tab(tmp: Path):
    """The «Develop» tab really built, on a temp actions dir and a cold runtime.
    ...
    from panel.tabs import develop as dev
    DevelopTab = dev.DevelopTab
...
def test_a_run_locks_the_list_marks_the_row_and_stops():
    ...
    try:
        ...
        marked = [tab._scn_list.get(i) for i in range(tab._scn_list.size())
                  if tab._scn_list.get(i).startswith(dev.RUNNING_MARK)]
        ...
        assert not any(tab._scn_list.get(i).startswith(dev.RUNNING_MARK)
